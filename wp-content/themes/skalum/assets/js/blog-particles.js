(function () {
  ...
  function buildConfig() {
    ...
    return {
      ...
      interactivity: {
        ...
        events: {
          onhover: {
            enable: !touch && !reduced,
            mode: "bubble"
          },
          onclick: {
            enable: !reduced && !isTiny,
            mode: "push"
          },
          resize: true
        },
  ...
  function destroyParticles() {
    const inst = window.pJSDom && window.pJSDom.find(
      (d) => d?.pJS?.canvas?.el?.id === EL_ID
    );
  ...
  function initParticles() {
    ...
    destroyParticles();
    window.particlesJS(EL_ID, buildConfig());
  }
  ...
  function setRunning(run) {
    ...
    if (inst.pJS.fn?.particlesRefresh) {
      inst.pJS.fn.particlesRefresh();
    }
  }
